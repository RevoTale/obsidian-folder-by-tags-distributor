...
export class GetAllTagsInTheVault extends Plugin {
	...
	constructor(app: App, manifest: PluginManifest) {
		...
		this.fileCache = this.fileArray.map((value) => this.app.metadataCache.getFileCache(value)).filter<CachedMetadata>((v): v is CachedMetadata=>!!v);
		this.tagArray = this.fileCache.map((value) => getAllTags(value)).filter((v):v is string[]=>v !== null);
